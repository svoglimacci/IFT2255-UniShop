...
public class UserMenu extends SubMenu {
    ...
    public UserMenu(User user, UserController userController, ProductController productController, User currentUser) {
...
if (currentUser instanceof Buyer && user instanceof Seller) {
    ...
    private Command createLikeCommand() {
        final String[] commandName = {
            currentUser instanceof Buyer && ((Buyer) currentUser).getLikedUsers()
                .contains(user.getId()) ? "Unlike" : "Like"};
        return new Command() {
            @Override
